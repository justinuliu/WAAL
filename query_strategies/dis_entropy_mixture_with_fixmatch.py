...
class FixMatchDisEntropyMixture:
    ...
    def query(self, query_num):
        ...
        # Querying
        query_num_part = int(query_num * self.args['repr_portion'])
        discriminator.eval()
        ...
        query_repr = idx_ulb_train[repr_probs.sort()[1][:query_num_part]]
        idx_ulb_train = [i for i in idx_ulb_train if i not in query_repr]
        idx_ulb_train = np.array(idx_ulb_train)
        ...
        idx = entropy_score.sort(descending=True)[1][:(query_num - query_num_part)]
        query_entropy = idx_ulb_train[idx]

        return np.concatenate((query_repr, query_entropy), axis=0)
